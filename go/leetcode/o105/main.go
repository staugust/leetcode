...
func minHeightShelvesHelper(books [][]int, idx, h, cw, shelf_width int, cache map[int]int) int {
	if idx >= len(books) {
		return h
	}
	if books[idx][0]+cw > shelf_width {
		if _, ok := cache[idx+1]; !ok {
			cache[idx+1] = minHeightShelvesHelper(books, idx+1, books[idx][1], books[idx][0], shelf_width, cache)
		}
		return h + cache[idx+1]
	}
	if books[idx][1] <= h {
		return minHeightShelvesHelper(books, idx+1, h, cw+books[idx][0], shelf_width, cache)
	}
	withinRow := minHeightShelvesHelper(books, idx+1, books[idx][1], cw+books[idx][0], shelf_width, cache)
	if _, ok := cache[idx+1]; !ok {
		cache[idx+1] = minHeightShelvesHelper(books, idx+1, books[idx][1], books[idx][0], shelf_width, cache)
	}
	newRow := h + cache[idx+1]
	if withinRow < newRow {
		return withinRow
	}
	return newRow
