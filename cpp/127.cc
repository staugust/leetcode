...
public:
	int ladderLength(string beginWord, string endWord, vector<string>& wordList) {
		...
		cnts[beginWord] = 1;
		while (!stk.empty()) {
			string word = stk.front();
			stk.pop();
			if ((words.count(word) == 0 && word != beginWord) || processed.count(word) != 0) {
				continue;
			}
	...
	int ladderLength_fast(const string& beginWord, const string& endWord, const vector<string>& wordList) {
